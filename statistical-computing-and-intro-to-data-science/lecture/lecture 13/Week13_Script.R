...
batting%>%
        ...
        summarize(max=max(HR,na.rm=TRUE))
...
new_data <- batting[batting$yearID>=1990,]
new_data <- new_data[,c("playerID",'HR')] 
new_data <- split(new_data,new_data$playerID)
my_average <- function(df){
        return(mean(df$HR,na.rm = TRUE))
}
new_data <- sapply(new_data,my_average)
new_data <- new_data[order(new_data,decreasing = TRUE)]
head(new_data,5)
# Check YourSelf
...
new_data <- salaries[batting$yearID==2010,]
new_data <- salaries[,c("teamID","salary")]
my_sum <- function(df){
        return(sum(df$salary,na.rm = TRUE))
}
result <- daply(new_data,.(new_data$teamID),my_sum)
