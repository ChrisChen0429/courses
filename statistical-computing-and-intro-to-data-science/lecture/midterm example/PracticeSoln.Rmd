...
```{r}
switch.digits <- function(digits){
  return(gsub("1|5|39", "0", digits))
}  
switch.digits("1234567890123456")
```
...
```{r}
stu <- read.csv("StudentData.csv", header = TRUE, as.is = TRUE)
```
...
```{r}
stu$LowestScore <- apply(stu[, 2:5], 1, min)
```
...
```{r}
for (i in 1:nrow(stu)) {
  stu$LowestScore2[i] <- min(stu[i, 2:5]) 
}    
```
...
```{r}
set.seed(1)
stu$Classroom <- sample(rep(c("309", "SIPA"), each = 25))
```
