...
my_average <- function(df){
        return(mean(df$unemployment,na.rm = TRUE))
}
mean.unemployment <- sapply(strikes.split,FUN = my_average)
mean.unemployment[which.max(mean.unemployment)]
mean.unemployment[which.min(mean.unemployment)]

corr <- function(df){
        return(cor(df$unemployment,df$left.parliament))
}

sapply(strikes.split,corr)
...
sapply(strikes.split,my.strike.lm)
...
library(ggplot2)
ggplot()+
        geom_line(aes(x=as.numeric(colnames(years.mat)),y=years.mat[1,],col='Unemployment'))+
        geom_line(aes(x=as.numeric(colnames(years.mat)),y=years.mat[2,],col='Inflation'))


plot(1:ncol(strike.coeff), strike.coeff[2, ], xaxt = "n",
     xlab = "Country", ylab = "Regression coefficient",
     main="Countrywise labor activity by leftwing score")
axis(side = 1, at = 1:ncol(strike.coeff),
     labels = colnames(strike.coeff), las = 2,
     cex.axis = 0.5)
abline(h = 0, col = "grey")
...
daply(strikes,.(country,I(year<=1975)),inflation.mean)
ddply(strikes,.(country,I(year<=1975)),inflation.mean)
