...
library(car)
rm_anova1 <- Anova(mod = mv1, idata = fac, idesign = ~ noise, type = 3)
summary(rm_anova1)
...
# Define a contrast as usual by psi = c1*Ang1 + c2*Ang2 + c3*Ang3. Each contrast 
...
# at the age = 1 level. That is, psi = 1*Ang1 - 1*Ang2 + 0*Ang3. Then, apply the
# contrast to the data.
psi_vals <- 1 * rmd2$angle0[rmd2$age == 1] - 1 * rmd2$angle4[rmd2$age == 1]
psi_vals
mean(psi_vals)
