...
```{r}
setwd("C:/Users/cheny/Desktop/study/second term/staistical machine learning/tutorial/tutorial two")
```
...
```
```{r}
scaled.3 <- as.matrix(scaled.3)
corvariance_matrix <- t(scaled.3)%*%scaled.3 
y <- eigen(corvariance_matrix)$vectors
par(mfrow=c(4,4), mai=c(0.1,0.1, 0.1, 0.1))
for(j in 1:16) {
  output.image(y[,j])
}
